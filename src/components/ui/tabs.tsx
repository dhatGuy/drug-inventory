...
>(({ className, ...props }, ref) => (
  <TabsPrimitive.List
    ref={ref}
    className={cn(
      "web:inline-flex h-16 items-center justify-center rounded-md bg-muted p-1 native:px-1.5",
      className
    )}
    {...props}
  />
...
>(({ className, ...props }, ref) => {
  ...
  return (
    <TextClassContext.Provider
      value={cn(
        "text-sm native:text-base font-PoppinsMedium text-muted-foreground web:transition-all",
        value === props.value && "text-foreground"
      )}>
      <TabsPrimitive.Trigger
        ref={ref}
        className={cn(
          `inline-flex items-center justify-center shadow-none web:whitespace-nowrap rounded-sm px-3
          py-1.5 text-sm font-medium web:ring-offset-background web:transition-all
